...
namespace mono
{
    ...
    struct AnimNotifyTriggerComponent
    {
        uint32_t anim_notify_hash;
        uint32_t completed_trigger_hash;
    };
    ...
    public:
        ...
        AnimNotifyTriggerComponent* AllocateAnimNotifyTrigger(uint32_t entity_id);
        void ReleaseAnimNotifyTrigger(uint32_t entity_id);
        void AddAnimNotifyTrigger(uint32_t entity_id, uint32_t anim_notify_hash, uint32_t completed_hash);
        ...
        template<typename T>
        void ForEeachAnimNotiryTrigger(T&& callable)
        {
            m_anim_notify_triggers.ForEach(callable);
        }
    ...
    private:
        ...
        mono::ActiveVector<AnimNotifyTriggerComponent> m_anim_notify_triggers;
