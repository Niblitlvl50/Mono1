...
SDLWindow::SDLWindow(int width, int height, unsigned int flags, IOGLSetupPtr setup)
    : mWidth(width),
      mHeight(height),
      mFlags(flags),
      mOGLSetup(setup)
{
    ...
    mWindow = SDL_CreateWindow("NIB", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, flags);
    if(!mWindow)
